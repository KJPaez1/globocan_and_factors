...
slm_edi_inc <- 
  ...
  group_by(cancer_type) |> 
  nest() |> 
  mutate(model = map(data, slm_edi))

summarized_ei = slm_edi_inc  |> 
...
slm_sdi_inc <- 
  ...
  group_by(cancer_type) |> 
  nest() |> 
  mutate(model = map(data, slm_sdi))

summarized_si = slm_sdi_inc  |> 
  mutate(summary = map(model, broom::glance)) %>%
  unnest(summary) |> select(-data, -model)
```
