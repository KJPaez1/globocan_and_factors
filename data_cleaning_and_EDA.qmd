...
Human Development Index (hdi), hdi composites, and socio demographic (sdi) index were retrieved from United Nations of the Development Program (https://hdr.undp.org/).
...
### Education and Income index (edi)
...
slm_edi_inc <- 
  ...
  mutate(model = map(data, function(data) {
    lm(asr_world ~ education_and_income, data = data)
  })) 

slm_edi_inc_m1 <- slm_edi_inc |>
...
slm_edi_inc_m2 <- slm_edi_inc |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_edi_inc_m3 <- bind_cols(slm_edi_inc_m1, slm_edi_inc_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
slm_edi_mort_m2 <- slm_edi_mort |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_edi_mort_m3 <- bind_cols(slm_edi_mort_m1, slm_edi_mort_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
slm_hdi_inc_m2 <- slm_hdi_inc |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_hdi_inc_m3 <- bind_cols(slm_hdi_inc_m1, slm_hdi_inc_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
slm_hdi_mort_m2 <- slm_hdi_mort |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_hdi_mort_m3 <- bind_cols(slm_hdi_mort_m1, slm_hdi_mort_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
slm_sdi_inc <- 
  ...
  nest() |> 
  mutate(model = map(data, function(data) {
    lm(asr_world ~ sdi_gdb_2021, data = data)
  })) 

slm_sdi_inc_m1 <- slm_sdi_inc |>
  mutate(summary = map(model, broom::tidy)) |>
  unnest(summary) |>
  filter(term != "(Intercept)") |>
  select(-data, -model, -term) |>
...
slm_sdi_inc_m2 <- slm_hdi_inc |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_sdi_inc_m3 <- bind_cols(slm_sdi_inc_m1, slm_sdi_inc_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
slm_sdi_mort_m2 <- slm_hdi_mort |>
  mutate(summary = map(model, broom::glance)) |>
  unnest(summary) |>
  select(-data, -model) |>
  mutate(across(where(is.numeric), round, 2))
         
slm_sdi_mort_m3 <- bind_cols(slm_sdi_mort_m1, slm_sdi_mort_m2) |>
  dplyr::select(
    cancer_type...1,
    estimate,
    r.squared,
...
my_palette_1 = c("#008ECE", "#00A9E0", "#59C7EB", "#A6E1F4", "#CCEEF9")
my_palette_2 = c("black", "#666666", "#999999", "#CCCCCC", "#E5E5E5")
```

Adjusting categories and removing NAs

```{r}
cancers_for_anova = globocan_5 |> 
  distinct(cancer_type, country_name, indicator, .keep_all = TRUE) |> 
  filter(cancer_type %in% c("Leukaemia" ,"Hodgkin lymphoma" ,"Non-Hodgkin lymphoma", 
                            " Brain, central nervous system", "Breast",
                            "Pancreas", "Kidney", "Colorectum", "Colon")) |>
  filter(edi_categories != "NA" & hdi_category != "NA" & sdi_categories != "NA" & sdi_categories != "") |> 
  mutate(edi_categories = 
           factor(edi_categories, 
                  levels = c("Low", "Medium", "High", "Very high")),
         hdi_category = 
           factor(hdi_category, 
                  levels = c("Low", "Medium", "High", "Very high")),
         sdi_categories = 
           factor(sdi_categories, 
...
```{r}
create_boxplot <- function(data, indicator, cancer_type, x_var, x_label, 
                           y_label, palette, x_margin = 20, y_margin = 10, 
                           plot_margin = c(0.8, 0.8, 0.8, 1), 
                           title = NULL) {
  p <- data %>%
    filter(indicator == !!indicator, cancer_type == !!cancer_type) %>%
    ggplot(aes(y = asr_world, x = .data[[x_var]], col = .data[[x_var]])) +
    labs(x = x_label, y = y_label) +
    geom_beeswarm(size = 4, alpha = 0.5) +
    theme_classic(base_size = 22) +
    theme(
      legend.position = "none",
      plot.margin = unit(plot_margin, "cm"),
      axis.title.x = element_text(margin = margin(t = x_margin)),
      axis.title.y = element_text(margin = margin(r = y_margin))
    ) +
    scale_colour_manual(values = palette)
  
  if (!is.null(title)) {
    p <- p + ggtitle(title) + theme(plot.title = element_text(size = 18, hjust = 0, colour = "#666666"))
  }
...
edi, hdi, sdi vs incidence Brain, central nervous system, Breast, Hodking lymphoma, non-hodking lymphoma

edi, hdi, sdi vs mortality Brain, central nervous system

hdi, sdi vs mortality kidney

hdi vs mortality colon

sdi vs mortality colorectum, pancreas

### Indicators vs Incidence

Brain, central nervous system, Breast, Hodking lymphoma, non-hodking lymphoma

```{r}
#Breast
boxplot_breast_inc_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Breast",
  x_var = "edi_categories",
  x_label = NULL,
  y_label = NULL,
...
boxplot_breast_inc_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Breast",
  x_var = "hdi_category",
...
boxplot_breast_inc_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Breast",
  x_var = "sdi_categories",
  x_label = NULL,
  y_label = NULL,
  palette = my_palette_1,
  title = " "
...
boxplot_bcns_inc_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = " Brain, central nervous system",
  x_var = "edi_categories",
  x_label = NULL,
  y_label = NULL,
  palette = my_palette_1,
  title = "Brain, central nervous system cancer"
)

boxplot_bcns_inc_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = " Brain, central nervous system",
  x_var = "hdi_category",
...
boxplot_bcns_inc_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = " Brain, central nervous system",
  x_var = "sdi_categories",
...
#Hodgkin lymphoma
boxplot_hodlym_inc_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
...
boxplot_hodlym_inc_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Hodgkin lymphoma",
  x_var = "hdi_category",
...
boxplot_hodlym_inc_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Hodgkin lymphoma",
  x_var = "sdi_categories",
...
boxplot_nonhodlym_inc_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "edi_categories",
  x_label = "Education and Income index",
  y_label = NULL,
  palette = my_palette_1,
  title = "Non-Hodgkin lymphoma"
)

boxplot_nonhodlym_inc_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "hdi_category",
...
boxplot_nonhodlym_inc_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "incidence",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "sdi_categories",
  x_label = "Sociodemographic Index",
  y_label = NULL,
  palette = my_palette_1,
  title = " "
)

boxplot_global_incid = 
  ggarrange(
    boxplot_breast_inc_edi, boxplot_breast_inc_hdi, boxplot_breast_inc_sdi,
    boxplot_bcns_inc_edi, boxplot_bcns_inc_hdi, boxplot_bcns_inc_sdi,
    boxplot_hodlym_inc_edi, boxplot_hodlym_inc_hdi, boxplot_hodlym_inc_sdi,
    boxplot_nonhodlym_inc_edi, boxplot_nonhodlym_inc_hdi, boxplot_nonhodlym_inc_sdi,
    ncol = 3,
    nrow = 4
    )

boxplot_global_incid = 
  annotate_figure(boxplot_global_incid,
                  left = text_grob("Age-Standarized Incidence Rate (per 100,000)", 
                                    color = "black", 
                                    rot = 90,
                                    size = 22))
```
...
boxplot_bcns_mort_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = " Brain, central nervous system",
  x_var = "edi_categories",
...
boxplot_bcns_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = " Brain, central nervous system",
  x_var = "hdi_category",
...
boxplot_bcns_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = " Brain, central nervous system",
  x_var = "sdi_categories",
...
#Kidney
boxplot_kidney_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Kidney",
  x_var = "hdi_category",
...
boxplot_kidney_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Kidney",
  x_var = "sdi_categories",
...
#Colon
boxplot_colon_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Colon",
  x_var = "hdi_category",
...
#Colorectum cancer
boxplot_colorec_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Colorectum",
  x_var = "sdi_categories",
  x_label = "Sociodemographic Index",
  y_label = NULL,
  palette = my_palette_2,
  title = "Colorectum cancer"
)

#Pancreas
boxplot_pancr_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
...
boxplot_global_mort = 
  ggarrange(
    boxplot_bcns_mort_hdi, boxplot_bcns_mort_sdi, boxplot_bcns_mort_edi,
    boxplot_kidney_mort_hdi, boxplot_kidney_mort_sdi,  
    boxplot_colorec_mort_sdi, boxplot_colon_mort_hdi, boxplot_pancr_mort_sdi,
    ncol = 3,
    nrow = 3
    )

boxplot_global_mort = 
  annotate_figure(boxplot_global_mort,
                  left = text_grob("Age-Standarized Mortality Rate (per 100,000)", 
                                    color = "black", 
                                    rot = 90,
                                    size = 22))
```
...
ggarrange(
  boxplot_bcns_inc_edi, boxplot_bcns_inc_hdi, boxplot_bcns_inc_sdi,
  boxplot_bcns_mort_edi, boxplot_bcns_mort_hdi, boxplot_bcns_mort_sdi
)
```
...
boxplot_hodlym_mort_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Hodgkin lymphoma",
  x_var = "edi_categories",
...
boxplot_hodlym_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Hodgkin lymphoma",
  x_var = "hdi_category",
...
boxplot_hodlym_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Hodgkin lymphoma",
  x_var = "sdi_categories",
...
ggarrange(
  boxplot_hodlym_inc_edi, boxplot_hodlym_inc_hdi, boxplot_hodlym_inc_sdi,
  boxplot_hodlym_mort_edi, boxplot_hodlym_mort_hdi, boxplot_hodlym_mort_sdi
)
```
...
boxplot_nonhodlym_mort_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "edi_categories",
...
boxplot_nonhodlym_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "hdi_category",
...
boxplot_nonhodlym_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Non-Hodgkin lymphoma",
  x_var = "sdi_categories",
...
ggarrange(
  boxplot_nonhodlym_inc_edi, boxplot_nonhodlym_inc_hdi, boxplot_nonhodlym_inc_sdi,
  boxplot_nonhodlym_mort_edi, boxplot_nonhodlym_mort_hdi, boxplot_nonhodlym_mort_sdi
)
```
...
boxplot_breast_mort_edi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Breast",
  x_var = "edi_categories",
...
boxplot_breast_mort_hdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Breast",
  x_var = "hdi_category",
...
boxplot_breast_mort_sdi <- create_boxplot(
  data = cancers_for_anova,
  indicator = "mortality",
  cancer_type = "Breast",
  x_var = "sdi_categories",
...
ggarrange(
  boxplot_breast_inc_edi, boxplot_breast_inc_hdi, boxplot_breast_inc_sdi,
  boxplot_breast_mort_edi, boxplot_breast_mort_hdi, boxplot_breast_mort_sdi
)
```
