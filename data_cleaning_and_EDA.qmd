...
cancers_for_anova = globocan_5 |> 
  distinct(cancer_type, country_name, indicator, .keep_all = TRUE) |> 
  filter(cancer_type %in% c("Leukaemia" ,"Hodgkin lymphoma" ,"Non-Hodgkin lymphoma", " Brain, central nervous system", "Breast")) |>
...
```{r}
create_boxplot <- function(data, indicator, cancer_type, x_var, x_label, y_label, palette, x_margin = 20, y_margin = 20, plot_margin = c(2, 1, 1, 1)) {
  data %>%
    filter(indicator == !!indicator, cancer_type == !!cancer_type) %>%
    ggplot(aes(y = asr_world, x = .data[[x_var]], col = .data[[x_var]])) +
    labs(x = x_label, y = y_label) +
    geom_beeswarm(size = 6, alpha = 0.8) +
