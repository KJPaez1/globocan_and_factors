...
cancer_code <- 
  read.csv("~/globocan_and_factors/raw_data/cancer_code.csv", stringsAsFactors = T)
...
Estimatin of EdI was based on the study of Lortet-Tieulent et al. (2020).

Set values were as follows: max_EYS = 20, max_MYS = 15, min_GNI = 100, max_GNI = 75000
...
hdi_and_comp_2022_0 <- hdi_and_comp_2022 |> 
  mutate(Education_and_Income = calculate_EdI(MYS = Mean_years_of_schooling_y,
                                              EYS = Expected_years_of_schooling_y,
                                              GNI_per_capita =
                                                GNI_per_capita_2017PPP_dolar),
...
globocan_2 <- globocan_2 |>
  mutate(
    ...
    tumor_types = case_when(
      ...
      cancer_type == "Liver and intrahepatic bile ducts" ~ "Hepatocellular/Adenocarcinoma",
...
# Mortality vs human_development_index
globocan_2 |>
  distinct(cancer_type, country_name, .keep_all = TRUE) |>
  filter(indicator == "mortality") |> 
...
# Incidence vs human_development_index
globocan_2 |>
  distinct(cancer_type, country_name, .keep_all = TRUE) |>
  filter(indicator == "incidence") |> 
  ggplot(aes(human_development_index, asr_world)) +
