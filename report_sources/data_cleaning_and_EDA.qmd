...
data_test <- globocan_5 |>
  dplyr::filter(cancer_type == "Breast" & indicator == "incidence")
```
...
abline(v=0.55, lty="dotted", col='tomato')
abline(v=0-85, lty="dotted", col='tomato')
text(x=0.55, y=0.3, 't=0.55', col='tomato')
text(x=0-85, y=0.3, 't=0.55', col='tomato')
```

```{r}
model_spline_1_knot <- lm(education_and_income ~ ns(asr_world, df = 1), data_test)

plot(data_test$asr_world, data_test$education_and_income, main = "Spline Regression")
lines(data_test$asr_world, predict(model_spline_1_knot), col = "red", lwd = 2)
