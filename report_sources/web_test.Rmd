...
# My function scripts
source(here("scripts", "Tidying data_Indicators cancers and countries.R"))
source(here("scripts", "ggplot themes.R"))
# library(rfextras)
...
# Generalized Additive Model (GAM) {data-orientation="rows"}
...
### Generalized Cross Validation (GCV) criterion for cubic regression splines
...
# data_x <-
#   globocan_8 |>
#   dplyr::filter(indicator == "Incidence" & cancer_type == "All cancers excl. non-melanoma skin cancer")
```
...
# attr(ns(data_x$hdi, df = 4), "knots") # k-2 df
```

# One-Way ANOVA tests {data-orientation="rows"}
...
renderPlotly({
  ...
  my_heatmap <- ggplot2::ggplot(
    ...
    ggplot2::geom_text(aes(label = sprintf("%.2f", R2)), color = "black", size = 2.7) +
    ...
    ggplot2::theme(
      ...
      axis.text.y = element_text(size = 10, color = "black", family = "Syne"),
